...
function attachCustomCommands(context, options) {
    var Cypress = context.Cypress, cy = context.cy, firebase = context.firebase, app = context.app;
    var defaultApp = app || firebase.app(); // select default  app
    /**
     ...
     * @name cy.login
     */
    Cypress.Commands.add((options && options.commandNames && options.commandNames.login) || 'login', function (uid, customClaims, tenantId) {
        if (tenantId === void 0) { tenantId = Cypress.env('TEST_TENANT_ID'); }
        var userUid = uid || Cypress.env('TEST_UID');
        // Handle UID which is passed in
        if (!userUid) {
            throw new Error('uid must be passed or TEST_UID set within environment to login');
        }
        var auth = getAuth(tenantId);
        ...
        return cy
            ...
            .then(function (customToken) { return loginWithCustomToken(auth, customToken); });
    });
    /**
    ...
    Cypress.Commands.add((options && options.commandNames && options.commandNames.callRtdb) ||
        'callRtdb', function (action, actionPath, dataOrOptions, options) {
        var taskSettings = {
            action: action,
            path: actionPath,
        };
        ...
        if (['set', 'update', 'push'].includes(action)) {
            ...
            var dataIsObject = getTypeStr(dataOrOptions) === 'object';
            var dataToWrite = dataIsObject ? tslib_1.__assign({}, dataOrOptions) : dataOrOptions;
    ...
    Cypress.Commands.add((options && options.commandNames && options.commandNames.callFirestore) ||
        'callFirestore', function (action, actionPath, dataOrOptions, options) {
        var taskSettings = {
            action: action,
            path: actionPath,
        };
    ...
    Cypress.Commands.add((options && options.commandNames && options.commandNames.getAuthUser) ||
        'getAuthUser', function (uid) { return cy.task('getAuthUser', uid); });
}
